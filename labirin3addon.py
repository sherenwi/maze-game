...
class Player(GameSprite):
    def __init__(self, player_image, player_x, player_y, size_x, size_y, player_x_speed, player_y_speed):
        ...
        self.image_left = transform.flip(self.image, True, False)
    ...
    def update(self):
        ...
        if self.x_speed > 0:  # Moving to the right
            self.image = self.image_right
            self.facing_right = True
            for p in platforms_touched:
                self.rect.right = min(self.rect.right, p.rect.left)
        elif self.x_speed < 0:  # Moving to the left
    ...
    def fire(self):
        if self.facing_right:
            ...
            bullet_direction = -15
        bullet = Bullet('bullet.png', bullet_x, self.rect.centery, 15, 20, bullet_direction)
        bullets.add(bullet)
